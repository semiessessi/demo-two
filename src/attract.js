...
export function createAttract(scene, camera, { ship, thrusters, chigKit, enemyMgr, projectiles, vfx, debris, lighting, rcs, rcsLights = 0 }) {
  const allies = [];
  ...
  const allyRcs = allies.map((a, i) => (i === 0 && rcs) ? rcs : createRcs(scene, { pivot: a.pivot, radius: ship.radius }, undefined, { lights: rcsLights }));
